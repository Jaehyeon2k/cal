...
import { fetchTimetable, upsertTimetable } from "../api/timetablesApi";
...
export default function TimetablePage() {
  ...
  useEffect(() => {
    ...
    fetchTimetable({ ownerUid: user.uid, term })
      .then((t) => {
        if (!alive) return;
        if (t) {
          setTimetableName(t.name || "시간표 없음");
          setSubjects(Array.isArray(t.subjects) ? t.subjects : []);
        } else {
          // 없으면 초기값 유지
  ...
  // ===== 자동 저장: 변경될 때마다 업서트 (너무 자주 저장되는 것 방지: 400ms 디바운스) =====
...
const resetAll = async () => {
  ...
  const blocksByDay = useMemo(() => {
    ...
    for (const arr of map) {
      arr.sort(
        (a, b) => a.startHour24 - b.startHour24 || a.endHour24 - b.endHour24
      );
    }
    return map;
  }, [subjects, dayToIndex]);
      ...
      <div className="tt-wrap">
        ...
        {!isLoggedIn && (
          <div style={{ opacity: 0.8, marginBottom: 12 }}>
            ⚠️ 로그인해야 시간표가 db.json에 저장됩니다. (비로그인 상태에서는
            저장 안 함)
          </div>
        )}
